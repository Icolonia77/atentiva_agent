...
# Cada dicionário representa um tópico com palavras-chave e uma resposta formatada.
KNOWLEDGE_BASE = [
    {
        'keywords': ['bermuda', 'roupa', 'vestir', 'chinelo', 'boné', 'vestimenta', 'traje'],
        'answer': "De acordo com as boas práticas da Atentiva, é necessário usar roupas adequadas, evitando regatas, bermudas, bonés e chinelos."
    },
    {
        'keywords': ['pagamento', 'ganhos', 'km', 'valor', 'reembolso', 'pedágio', 'remuneração'],
        'answer': (
            "A remuneração na Atentiva funciona da seguinte forma:\n\n"
            "* **Valor por KM:** R$ 1,10 por quilómetro rodado.\n"
            "* **Corrida Integral:** A Atentiva paga a ida e a volta da corrida, mesmo sem passageiro no retorno.\n"
            "* **Valor Mínimo:** Corridas de até 40 km (ida e volta) têm um valor fixo de R$ 44,00.\n"
            "* **Reembolso de Pedágio:** Todos os pedágios, tanto na ida quanto na volta, são reembolsados.\n"
            "* **Adicional de Longa Distância:** Há um bónus de R$ 80,00 a cada 1000 km rodados numa única corrida.\n"
            "* **Horas Paradas (HP):** Pagamento de R$ 17,00 por hora parada, geralmente aplicável quando se espera pelo guincho no local de embarque."
        )
    },
    {
        'keywords': ['carro', 'veículo', 'ano', 'modelo', 'calota', 'porta', 'requisitos do carro'],
        'answer': (
            "Os requisitos para o veículo são:\n\n"
            "* **Idade:** Não pode ultrapassar 12 anos de uso.\n"
            "* **Características:** Precisa ter 4 portas e ar-condicionado a funcionar perfeitamente ('gelando').\n"
            "* **Aparência:** Deve estar sempre limpo e bem cuidado. Se as rodas forem de ferro, é obrigatório o uso de calotas."
        )
    },
    {
        'keywords': ['seguro', 'acidente', 'apólice'],
        'answer': (
            "Sim, a adesão ao seguro corporativo do aplicativo é **obrigatória**. Veja os detalhes:\n\n"
            "* **Obrigatoriedade:** É uma imposição das seguradoras com as quais a Atentiva trabalha.\n"
            "* **Cobertura:** Oferece cobertura de até R$ 100.000,00 para socorro médico e hospitalar em caso de acidentes, válida para pessoas (motorista e passageiros).\n"
            "* **Custo:** O valor anual é de R$ 200,00, mas o parceiro paga apenas **R$ 100,00**, com a Atentiva a pagar a outra metade. Este valor é debitado no primeiro faturamento, não sendo pago adiantado."
        )
    },
    {
        'keywords': ['mei', 'cnpj', 'nota fiscal', 'nf'],
        'answer': "Sim, é necessário ter um MEI ou CNPJ para a emissão de Nota Fiscal (NF). No entanto, é possível iniciar o cadastro como pessoa física e regularizar a situação depois. Caso não consiga emitir NF, a Atentiva oferece a opção de emissão por terceiros para não perder a parceria."
    },
    {
        'keywords': ['horário', 'exclusividade', 'disponibilidade', 'outro aplicativo'],
        'answer': "A Atentiva **não exige exclusividade**. O parceiro tem total flexibilidade para definir seus próprios horários, ligando e desligando o aplicativo conforme sua disponibilidade, e pode trabalhar com outros aplicativos simultaneamente."
    },
    {
        'keywords': ['atentiva', 'empresa', 'quem é', 'o que é', 'sobre'],
        'answer': "A ATENTIVA é uma empresa consolidada no setor de transportes, especializada no transporte de segurados. Ela atua no mercado desde 2013 e sua sede, que opera 24/7, fica em Sumaré/SP."
    }
]

def get_faq_answer(query):
    """
    Busca na base de conhecimento uma resposta correspondente às palavras-chave da pergunta.
    """
    query_lower = query.lower()

    # Itera sobre cada item da base de conhecimento
    for item in KNOWLEDGE_BASE:
        # Verifica se alguma das palavras-chave do item está na pergunta do usuário
        for keyword in item['keywords']:
            if keyword in query_lower:
                return item['answer']
    
    # Se o loop terminar sem encontrar nenhuma correspondência
    return "Não encontrei uma resposta para sua pergunta. Por favor, tente outras palavras ou entre em contato com Ricardo Ferreiro no número 19 99686-8581."
